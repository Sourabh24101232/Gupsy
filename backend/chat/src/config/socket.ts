...
export const isUserInChatRoom = (userId: string, chatId: string): boolean => {
  return [...(userSocketMap[userId] ?? [])].some((socketId) =>
    io.sockets.sockets.get(socketId)?.rooms.has(chatId),
  );
};
...
io.on("connection", (socket: Socket) => {
  ...
  socket.on("typing", (data: { chatId?: string }) => {
    if (!userId || !data.chatId || !socket.rooms.has(data.chatId)) return;
  ...
  socket.on("joinChat", async (chatId: string) => {
    if (!userId || !mongoose.isValidObjectId(chatId)) return;
    const chat = await Chat.findOne({ _id: chatId, users: userId }).select("_id");
    if (!chat) return;
    socket.join(chatId);
